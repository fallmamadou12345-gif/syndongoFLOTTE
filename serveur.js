...
// Extrait la date de prise de vue (EXIF DateTimeOriginal) et la position GPS d'une photo JPEG
// encodée en data URI. Pur JS, sans dépendance — retourne null si pas de JPEG/EXIF exploitable.
function extractExifDateAndGps(dataUri) {
  try {
    const match = /^data:image\/jpeg;base64,(.+)$/.exec(dataUri || '');
    ...
    while (offset < buf.length - 4) {
      ...
      if (marker === 0xE1) {
        ...
        if (buf.slice(segStart, segStart + 6).toString('ascii') === 'Exif\0\0') {
          exifBuf = buf.slice(segStart + 6, offset + 2 + segLen);
          break;
        }
      }
    ...
    const littleEndian = exifBuf.toString('ascii', 0, 2) === 'II';
    const readU16 = (o) => littleEndian ? exifBuf.readUInt16LE(o) : exifBuf.readUInt16BE(o);
    const readU32 = (o) => littleEndian ? exifBuf.readUInt32LE(o) : exifBuf.readUInt32BE(o);
    ...
    function readIFD(ifdOffset) {
      ...
      for (let i = 0; i < numEntries; i++) {
        ...
        entries[tag] = { type, count, valueOffset: entryOffset + 8 };
      }
      return { entries };
    }
    ...
    function readString(entry) {
      ...
      if (len > 4) dataOffset = readU32(entry.valueOffset);
      return exifBuf.toString('ascii', dataOffset, dataOffset + len - 1);
    }

    function readRational(offsetPos) {
      const num = readU32(offsetPos);
      const den = readU32(offsetPos + 4);
    ...
    if (ifd0.entries[0x8769]) {
      const exifIfdOffset = readU32(ifd0.entries[0x8769].valueOffset);
      const exifIfd = readIFD(exifIfdOffset);
      if (exifIfd.entries[0x9003]) {
        dateTimeOriginal = readString(exifIfd.entries[0x9003]);
      } else if (exifIfd.entries[0x9004]) {
        dateTimeOriginal = readString(exifIfd.entries[0x9004]);
      }
    }
    if (!dateTimeOriginal && ifd0.entries[0x0132]) {
      dateTimeOriginal = readString(ifd0.entries[0x0132]);
    }

    if (ifd0.entries[0x8825]) {
      const gpsIfdOffset = readU32(ifd0.entries[0x8825].valueOffset);
      const gpsIfd = readIFD(gpsIfdOffset);
      function readGpsCoord(refTag, coordTag) {
        if (!gpsIfd.entries[coordTag] || !gpsIfd.entries[refTag]) return null;
        const coordEntry = gpsIfd.entries[coordTag];
        let base = coordEntry.count > 1 ? readU32(coordEntry.valueOffset) : coordEntry.valueOffset;
        const deg = readRational(base);
        const min = readRational(base + 8);
        const sec = readRational(base + 16);
        let val = deg + min / 60 + sec / 3600;
        const refEntry = gpsIfd.entries[refTag];
        const ref = exifBuf.toString('ascii', refEntry.valueOffset, refEntry.valueOffset + 1);
        if (ref === 'S' || ref === 'W') val = -val;
        return val;
      }
      const lat = readGpsCoord(0x0001, 0x0002);
      const lon = readGpsCoord(0x0003, 0x0004);
    ...
    if (dateTimeOriginal) {
      ...
      if (m) {
        takenAt = new Date(Date.UTC(+m[1], +m[2] - 1, +m[3], +m[4], +m[5], +m[6]));
      }
    }
    return { takenAt, gps };
  } catch (e) {
...
async function handleAPI(req, res, body) {
  ...
  if(p==='/api/livreur/controle'&&method==='POST'){
    ...
    // Anti-fraude : la photo doit avoir été prise récemment (date EXIF), pas une ancienne photo réutilisée
    const now=new Date();
    const photosMeta={};
    for(const k of PHOTO_KEYS){
      const exif=extractExifDateAndGps(photos[k].data);
      photosMeta[k]={taken_at:exif&&exif.takenAt?exif.takenAt.toISOString():null,gps:exif&&exif.gps?exif.gps:null};
      if(exif&&exif.takenAt){
        const diffH=Math.abs(now-exif.takenAt)/3600000;
        if(diffH>6){
          return res.end(JSON.stringify({detail:'La photo "'+PHOTO_LABELS[k]+'" semble avoir été prise le '+exif.takenAt.toISOString().slice(0,16).replace('T',' ')+', ce qui est trop ancien. Merci de reprendre une photo à l\'instant.'}));
        }
      }
    }
    const photosSaved={};
    ...
    const c={id:uid(),vehicule_id:aff.vehicule_id,chauffeur_id:lvId,date_soumission:today(),
      ...
      photos:photosSaved,photos_hashes:hashesActuels,photos_meta:photosMeta,statut:'en_attente',
